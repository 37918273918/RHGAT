...
for seed in range(10):
    ...
    def construct_placeholders(edge_types):
        placeholders = {
            'batch': tf.compat.v1.placeholder(tf.compat.v1.int64, name='batch'),
            'batch_edge_type_idx': tf.compat.v1.placeholder(tf.compat.v1.int64, shape=(), name='batch_edge_type_idx'),
            'batch_row_edge_type': tf.compat.v1.placeholder(tf.compat.v1.int64, shape=(), name='batch_row_edge_type'),
            'batch_col_edge_type': tf.compat.v1.placeholder(tf.compat.v1.int64, shape=(), name='batch_col_edge_type'),
            'degrees': tf.compat.v1.placeholder(tf.compat.v1.int64),
            'dropout': tf.compat.v1.placeholder_with_default(0., shape=()),
        }
        placeholders.update({
            'adj_mats_%d,%d,%d' % (i, j, k): tf.compat.v1.sparse_placeholder(tf.compat.v1.float32)
            for i, j in edge_types for k in range(edge_types[i,j])})
        placeholders.update({
            'feat_%d' % i: tf.compat.v1.sparse_placeholder(tf.compat.v1.float32) for i, _ in edge_types})
        return placeholders
